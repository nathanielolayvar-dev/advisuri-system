    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 text-center">
          ...
          <button
            onClick={onClose}
            className="px-6 py-2 bg-slate-100 rounded-lg"
          >
            Close
          </button>
        </div>
              ...
              {dropdownOpen && (
                <div className="absolute top-full mt-2 w-full bg-white border rounded-xl shadow-xl z-50 max-h-48 overflow-y-auto">
                  {selectedStudents.map((student) => (
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded border flex items-center justify-center ${student.selected ? 'bg-blue-500 border-blue-500' : 'border-slate-300'}`}
                      >
                        {student.selected && (
