...
export const TasksView = ({ groupId, isStaff, userId }: TasksViewProps) => {
  ...
  const [newSubtasks, setNewSubtasks] = useState<
    { title: string; assigned_to: string }[]
  >([]);
    ...
    if (!error) {
      ...
      const formattedTasks = allTasks.map((task: any) => {
        const taskSubtasks = allTasks.filter(
          (t: any) => t.parent_task_id === task.id
        );
        ...
        const assignedUserName = task.assigned_to
          ? userMap.get(task.assigned_to) || null
          : null;
    ...
    try {
      const currentMaxVersion =
        submissions.length > 0
          ? Math.max(...submissions.map((s) => s.version_number))
          : 0;
      ...
      if (createdSubmissionId) {
        console.log(
          'Upload failed. Rolling back empty submission container...'
        );
        await supabase
          .from('submissions')
          .delete()
      ...
      setSelectedTask((prev) =>
        prev
          ? {
              ...
              progress_percentage:
                taskUpdatePayload.progress_percentage ||
      ...
      setSelectedTask((prev) =>
        prev
          ? {
              ...prev,
              ...editFields,
              priority: editFields.priority as 'low' | 'medium' | 'high',
            }
          : null
      );
      setEditTask(null);
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    if (!error) {
      setSelectedTask({
        ...
        progress_percentage:
          newStatus === 'completed' ? 100 : selectedTask.progress_percentage,
        completed_at:
          newStatus === 'completed'
  ...
  const updateParentTaskProgress = async (parentTaskId: string) => {
    try {
      ...
      if (updateError) {
        ...
        console.log(
          `Updated parent task ${parentTaskId} progress to ${progressPercentage}%${progressPercentage === 100 ? ' (marked as completed)' : ''}`
        );
      }
    } catch (error) {
  ...
  const handleDeleteTask = async () => {
    ...
    if (
      window.confirm(
        'Are you sure you want to delete this task AND all its submitted files? This cannot be undone.'
      )
    ) {
      ...
      try {
        ...
        if (subs && subs.length > 0) {
          ...
          if (attachments && attachments.length > 0) {
            ...
            if (storageError) {
              console.error(
                'Warning: Failed to delete some physical files.',
                storageError
              );
  ...
  if (selectedTask) {
    ...
    return (
      <div className="flex w-full h-full bg-slate-50 overflow-hidden">
        ...
        <div
          className={`flex-1 overflow-y-auto p-6 transition-all duration-300 ${activeGradingSub ? 'pr-4' : ''}`}
        >
          <div className="w-full bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <button
              onClick={goBack}
              className="mb-4 text-blue-600 text-sm flex items-center gap-1 font-medium hover:underline"
            >
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {/* @ts-ignore */}
                {selectedTask.final_score !== null &&
                  selectedTask.final_score !== undefined && (
            ...
            <div className="flex items-center gap-3 mb-6 text-xs">
              <span className="flex items-center gap-1.5 text-slate-500 font-medium">
                ...
                {selectedTask.due_date
                  ? `Due ${new Date(selectedTask.due_date).toLocaleString([], { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}`
                  : 'No Due Date'}
              </span>
              ...
              {selectedTask.priority && (
                <span
                  className={`px-2.5 py-0.5 rounded-full font-semibold capitalize ${selectedTask.priority === 'high' ? 'bg-red-50 text-red-600' : selectedTask.priority === 'medium' ? 'bg-amber-50 text-amber-600' : 'bg-slate-100 text-slate-600'}`}
                >
                  {selectedTask.priority}
                </span>
              )}
            ...
            {!isTeacher && (
              <div className="mb-6">
                ...
                <div className="border border-slate-200 rounded-lg p-4 bg-white">
                  ...
                  >
                    <Paperclip
                      className={`w-8 h-8 mb-2 ${isDragging ? 'text-blue-500' : 'text-slate-400'}`}
                    />
            ...
            <div className="mb-6">
              ...
              <div className="notes-grid">
                {notes.map((note) => (
                  <div
                    key={note.id}
                    className="note-card p-4 border border-slate-200 rounded-lg mb-2 bg-slate-50 relative group"
                  >
                    <p className="text-sm text-slate-800 whitespace-pre-wrap pr-6">
                      {note.content}
                    </p>
            ...
            <div className="mb-6 mt-8">
              ...
              <div className="space-y-4">
                {submissions.map((sub) => (
                  <div
                    key={sub.id}
                    className={`border rounded-lg p-4 transition-colors ${activeGradingSub?.id === sub.id ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-100' : 'border-slate-200 bg-white'}`}
                  >
                    {/* Header: Attempt Number & Status Badges */}
                    <div className="flex justify-between items-center mb-3">
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-slate-700">
                          Attempt {sub.version_number}
                        </span>
                    ...
                    <div className="space-y-2 bg-slate-50 p-3 rounded border border-slate-100">
                      {sub.attachments?.map((att: any) => (
                        ...
                        >
                          ...
                          <a
                            href={`https://behbluflerhbslixhywa.supabase.co/storage/v1/object/public/capstone_submissions/${att.file_url}`}
                            target="_blank"
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              <div className="mb-6">
                ...
                <div className="text-lg font-semibold text-slate-700 mb-2">
                  Attempt {activeGradingSub.version_number}
                </div>
              </div>
      ...
      <div className="w-full">
          ...
          <div className="mb-8 p-6 bg-white border border-slate-200 rounded-xl shadow-sm">
            ...
            <form onSubmit={handleCreateTask} className="space-y-4">
              ...
              <div className="border-t border-slate-200 pt-4 mt-4">
                    ...
                    {newSubtasks.map((subtask, index) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const updated = newSubtasks.filter(
                              (_, i) => i !== index
                            );
                            setNewSubtasks(updated);
                          }}
                ...
                {newSubtasks.length === 0 && (
                  <p className="text-sm text-slate-400 italic">
                    Click "Add Subtask" to create subtasks with individual
                    assignments
                  </p>
                )}
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                >
                  {newSubtasks.length > 0
                    ? `Create Task (${newSubtasks.filter((st) => st.title).length} subtasks)`
                    : 'Create Task'}
                </button>
              </div>
            </form>
        ...
        <div className="flex flex-col gap-4">
          ...
          {tasks
            ...
            .map((task) => {
              const taskSubtasks = tasks.filter(
                (st) => st.parent_task_id === task.id
              );
              return (
                <div key={task.id} className="space-y-2">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 text-xs text-slate-500 border-t border-slate-100 pt-3 mt-1">
                      ...
                      {task.priority && (
                        <span
                          className={`ml-auto px-2.5 py-0.5 rounded-full font-medium capitalize
                        ${
                          task.priority === 'high'
